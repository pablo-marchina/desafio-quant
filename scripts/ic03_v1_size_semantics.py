...
FEE_COMMIT="1a3c31c48275a9adceb039a05cfcf15aba4629bc"
ABI_URL=f"https://raw.githubusercontent.com/Polymarket/exchange-fee-module/{FEE_COMMIT}/artifacts/FeeModule.json"
...
def post(url,payload,timeout=12):
 req=urllib.request.Request(url,data=json.dumps(payload,separators=(",",":")).encode(),headers={"Content-Type":"application/json","User-Agent":"ARGOS-IC03-SIZE/2.0"})
 with urllib.request.urlopen(req,timeout=timeout) as r:return json.loads(r.read())
def fetch_url(url):
 req=urllib.request.Request(url,headers={"User-Agent":"ARGOS-IC03-SIZE/2.0"})
...
def main():
 ...
 raw=fetch_url(ABI_URL);(out/"FeeModule_pinned.json").write_bytes(raw);contract=Web3().eth.contract(address=Web3.to_checksum_address(FEE_MODULE),abi=json.loads(raw)["abi"])
