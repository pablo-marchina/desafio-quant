...
def validate_input(rows,c):
    ...
    for r in rows:
        ...
        seen.add(key)
        r['_anchor']=parse_anchor(r.get('anchor',''),e); r['_ecg']=e
    ...
    expected={(f,d) for f in fams for d in dims}
    if seen!=expected: raise ValueError('family x dimension grid incomplete')

def draw_value(anchor,ecg,rng):
    if ecg=='D': return rng.uniform(0,5)
    h=HALF[ecg]; lo=max(0.0,anchor-h); hi=min(5.0,anchor+h)
...
def run_smaa(families,n,seed):
    rng=random.Random(seed); names=sorted(families); k=len(names)
    sums={x:0.0 for x in names}; high={x:0 for x in names}; ranks={x:[0]*k for x in names}
    for _ in range(n):
        g=[rng.expovariate(1.0) for _ in DIMS]; z=sum(g); w=[v/z for v in g]
        sc={}
        for name in names:
            f=families[name]; vals=[draw_value(f['anchors'][d],f['ecg'][d],rng) for d in DIMS]
            s=sum(a*b for a,b in zip(w,vals)); sc[name]=s; sums[name]+=s; high[name]+=int(s>=3.0)
        order=sorted(names,key=lambda x:(-sc[x],x))
        for i,name in enumerate(order): ranks[name][i]+=1
    out={}
    for name in names:
        vec=[v/n for v in ranks[name]]
        out[name]={'mean_simulated_IAS':sums[name]/n,'P_IAS_ge_3':high[name]/n,'rank1_acceptability':vec[0],'rank_le2_acceptability':sum(vec[:2]),'rank_acceptability_vector':vec}
    return out
...
def comparative(families,stats):
    ...
    q.sort(key=lambda x:(-stats[x]['rank1_acceptability'],x)); a,b=q[:2]; margin=stats[a]['rank1_acceptability']-stats[b]['rank1_acceptability']
    permitted=stats[a]['rank1_acceptability']>=0.50 and margin>=0.05
...
def main():
    c=load_json(CONTRACT); p=load_json(PROTO)
    if c['execution_authorized'] is not True: raise SystemExit('real IAS scoring not authorized by contract')
    ...
    for f in fam:
        fam[f]['IAS_central']=central(fam[f]['anchors'],fam[f]['ecg']); fam[f]['evidence_gate']=evidence_gate(fam[f]['ecg'])
