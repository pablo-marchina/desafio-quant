...
def main():
    ...
    with (REG / "w4b_kalshi_semantic_series_v1.csv").open("w", newline="", encoding="utf-8") as f:
    ...
    write_csv_gz(REG / "w4b_kalshi_semantic_events_v1.csv.gz", event_rows, fields)
    write_csv_gz(REG / "w4b_kalshi_semantic_ambiguous_v1.csv.gz", ambiguous, fields + ["semantic_text"])

    summary = {
        "artifact":"W4B_KALSHI_SEMANTIC_SUMMARY","version":"W4B-KSS-v1.0","date_utc":datetime.now(timezone.utc).isoformat(),
        "protocol_version":PROTO["version"],"performance_blind":True,"linked_asset_realized_returns_read":False,
        "raw_classified_unique_series":len(by_series),"boundary_candidate_series":len(candidate_series),
        "boundary_rejected_series":len(by_series)-len(candidate_series),"event_rows_examined":len(event_rows),
        "accepted_strict_event_rows":len(accepted),"accepted_unique_canonical_events":len(unique_ids),
    ...
    (REG / "w4b_kalshi_semantic_summary_v1.json").write_text(json.dumps(summary, indent=2, sort_keys=True)+"\n")
    print(json.dumps({k:summary[k] for k in ["raw_classified_unique_series","boundary_candidate_series","event_rows_examined","accepted_strict_event_rows","accepted_unique_canonical_events","ambiguous_rows","api_error_count","route_truncation_count","gate_decision"]}, indent=2, sort_keys=True))
