...
def fam(title,slug=''):
 text=sv.classification_text({'title':title,'slug':slug})
 return sv.resolve(sv.strict_matches(text))
positives=[
...
text=sv.classification_text(row);ok('cpi' not in text.lower());st,f=sv.resolve(sv.strict_matches(text));ok(not f,(st,f))
...
summary={'artifact':'W2C_SEMANTIC_V2_SYNTHETIC_VALIDATION','version':'W2C-SV-v2-SYN-v1.2','passed':passed,'failed':0,'status':f'PASS_{passed}_OF_{passed}','real_candidate_data_read':False,'performance_data_read':False,'f1_f9_read':False,'ias_read':False,'strict_runner':'scripts/w2c_semantic_validation_v2_strict.py','test_interface':'classification_text_then_strict_matches'}
