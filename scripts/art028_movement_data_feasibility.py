...
def hhi_by_wallet(rows):
    ...
    tot = sum(by.values())
    return sum((v / tot) ** 2 for v in by.values()) if tot > 0 else None
...
FEATURE_SPECS = [
    ...
    ("F06", "H2_FLOW_PERSISTENCE", "CORE", "same_direction_transition_share_lifecycle", 105,
     "Full pre-cutoff market-lifecycle share of adjacent event-oriented trade directions that are equal; >=10 trades. This replaces the initial trailing-24h representation after label-free coverage review; the 24h statistic remains diagnostic."),
    ...
    ("F10", "H2_PATTERN_NOVELTY", "CHALLENGER", "matrix_profile_discord_6h", 80,
     "Minimum z-normalized Euclidean distance of latest six hourly changes to earlier 6h subsequences in the available <=72h pre-cutoff path; >=4 comparison windows."),
    ("F11", "H2_SEQUENTIAL_EVIDENCE", "CHALLENGER", "conformal_log_martingale_6h", 100,
     "Maximum cumulative log conformal betting evidence across last six absolute hourly changes versus prior [-30h,-6h] changes; epsilon fixed at 0.5."),
    ("F12", "H2_MULTIVARIATE_ANOMALY", "CHALLENGER", "multivariate_anomaly_distance", 70,
     "Prequential robust Euclidean distance across complete core movement components standardized using earlier events only; >=20 prior complete events."),
    ("F13", "H2_TRAJECTORY", "ROBUSTNESS", "sign_consistency_1_6_24h", 100,
     "Absolute mean sign across 1h, 6h and 24h probability changes; sign-only robustness."),
    ("F14", "H2_PRICE_DYNAMICS", "CONDITIONAL", "post_jump_half_life_hours", 40,
     "Half-life after fixed >=3 robust-scale hourly jump in last 24h with >=3h remaining before cutoff; baseline scale from [-72h,-24h]."),
    ...
    ("M02", "H2_FORECAST_DISAGREEMENT", "MODEL_LEVEL", "", 0,
     "Dispersion across simple forecasters is model-output dependent and remains protocol-only in ART-028."),
    ("R01", "H2_CALIBRATION", "MODEL_LEVEL_ROBUSTNESS", "", 0,
     "Calibration requires prior resolved labels; it remains an ART-029/030 robustness rule."),
    ("R02", "H2_DRIFT", "ROBUSTNESS", "prequential_feature_distance", 70,
     "Prequential robust distance of current complete core-feature vector from earlier-event distribution; drift/coverage monitor only."),
]
...
def main():
    ...
    for e in ordered:
        ...
        out = {"market_id": e["market_id"], "event_key": key, "ticker": e["ticker"],
            ...
            if s48:
                ...
                if cand:
                    ...
                    if score >= 3.0:
                        ...
                        pre, post = path72[48 + j], path72[49 + j]
                        mag = abs(post - pre)
                        if mag > EPS:
        ...
        out["trade_count_lifecycle"] = len(tr)
        ...
        if len(tr) >= 10:
            same, run = persistence(tr)
            out["same_direction_transition_share_lifecycle"] = same
            out["max_direction_run_share_lifecycle"] = run
        else:
            out["same_direction_transition_share_lifecycle"] = None
            out["max_direction_run_share_lifecycle"] = None

        q90 = quantile(prior_trade_notionals, .90) if len(prior_trade_notionals) >= 200 else None
    ...
    prior_model = []
    prior_core_vectors = []
    core_cols = ["delta_6h", "vol_scaled_delta_6h", "signed_notional_imbalance_24h", "wallet_hhi_notional_24h", "same_direction_transition_share_lifecycle", "jump_score_6h"]
    for out in rows:
        ...
        if all(v is not None for v in xvals):
            ...
            if len(prior_model) >= 20:
                X, y = [z[0] for z in prior_model], [z[1] for z in prior_model]
                beta = ridge_fit(X, y)
                if beta is not None:
                    resid = out["delta_6h"] - dot(beta, x)
                    rs = mad_scale([yy - dot(beta, xx) for xx, yy in prior_model])
                    if rs:
        ...
        if all(v is not None for v in vec):
            if len(prior_core_vectors) >= 20:
                ...
                for j, v in enumerate(vec):
                    ...
                    med, s = statistics.median(hist), mad_scale(hist)
                    if s:
    ...
    write_csv(matrix_csv, [{k: pct(v) if isinstance(v, float) else v for k, v in r.items()} for r in rows], list(rows[0]))
    ...
    spec_rows, coverage_rows = [], []
    for fid, fam, role, col, mincov, definition in FEATURE_SPECS:
        if not col:
            n, status = 0, "PROTOCOL_ONLY_NOT_LABEL_FREE_MATERIALIZABLE"
        else:
            n = sum(r.get(col) is not None and (not isinstance(r.get(col), float) or math.isfinite(r[col])) for r in rows)
            status = "PASS_COVERAGE" if n >= mincov else "FAIL_COVERAGE"
        spec_rows.append({"feature_id": fid, "architecture_family": fam, "role": role, "primary_column": col,
                          "minimum_event_coverage": mincov, "definition": definition,
                          "materialized_event_count": n, "coverage_status": status})
        coverage_rows.append({"feature_id": fid, "architecture_family": fam, "role": role, "primary_column": col,
    ...
    for col in materialized_cols:
        vals = [float(r[col]) for r in rows if r.get(col) is not None and math.isfinite(float(r[col]))]
        if vals:
            dist_rows.append({"feature": col, "n": len(vals), "mean": pct(statistics.mean(vals)),
                              "sd": pct(statistics.pstdev(vals) if len(vals) > 1 else 0.0), "min": pct(min(vals)),
                              "p05": pct(quantile(vals, .05)), "p25": pct(quantile(vals, .25)), "median": pct(statistics.median(vals)),
                              "p75": pct(quantile(vals, .75)), "p95": pct(quantile(vals, .95)), "max": pct(max(vals)),
        ...
        if v1 and v2:
            ...
            ks = ks_stat(v1, v2)
            flag = "STRONG_ERA_DISTRIBUTION_SHIFT" if ((shift is not None and shift > 1) or (ks is not None and ks > .50)) else ("MODERATE_ERA_DISTRIBUTION_SHIFT" if ((shift is not None and shift > .50) or (ks is not None and ks > .35)) else "NO_LARGE_ERA_SHIFT_DETECTED")
            era_rows.append({"feature": col, "n_v1": len(v1), "n_v2": len(v2), "median_v1": pct(statistics.median(v1)),
                             "median_v2": pct(statistics.median(v2)), "median_shift_over_pooled_iqr": pct(shift),
                             "ks_statistic": pct(ks), "flag": flag,
                             "interpretation": "Descriptive era/time check only; V1/V2 is confounded with calendar time and market maturity."})
    ...
    for a, b in itertools.combinations(materialized_cols, 2):
        ...
        if len(pairs) < 20:
            continue
        aa, bb = zip(*pairs)
        sr, pr = spearman(list(aa), list(bb)), pearson(list(aa), list(bb))
        corr_rows.append({"feature_a": a, "feature_b": b, "n_overlap": len(pairs), "spearman_rho": pct(sr),
                          "pearson_r": pct(pr), "abs_spearman": pct(abs(sr) if sr is not None else None),
    ...
    persistence_24h_n = sum(r.get("same_direction_transition_share_24h") is not None for r in rows)
    persistence_lifecycle_n = sum(r.get("same_direction_transition_share_lifecycle") is not None for r in rows)
    decision = "PASS_ART028_MOVEMENT_DATA_FEASIBILITY_ALL_CORE_FAMILIES_MATERIALIZED" if len(core_pass) == 7 else "REVIEW_ART028_CORE_COVERAGE"

    summary = {
        "decision": decision,
        "boundary": "ART-028 uses only pre-cutoff IC-03/IC-04 inputs and reads no outcomes, candidate predictive performance, equity returns or H2 scores.",
        "events_total": 117, "structurally_available_events": sum(r["structurally_available"] == "true" for r in rows),
        "structurally_unavailable_events": [r["event_key"] for r in rows if r["structurally_available"] != "true"],
        "core_feature_families_required": 7, "core_feature_families_passing_coverage": len(core_pass), "core_feature_ids_passing": core_pass,
        "challenger_feature_ids_passing": challenger_pass, "robustness_feature_ids_passing": robust_pass,
        ...
        "label_free_protocol_amendments": [{
            "feature_id": "F06", "reason": "Initial trailing-24h persistence representation failed its predeclared coverage gate in the first outcome-blind run. The minimum was not lowered. The representative was changed to full pre-cutoff lifecycle persistence, which is the natural sequence domain for order-splitting/persistence and uses the already-audited >=10-trade lifecycle coverage.",
            "initial_24h_materialized_events": persistence_24h_n, "revised_lifecycle_materialized_events": persistence_lifecycle_n,
            "outcomes_consulted": False
        }],
        "half_life_conditional_status": half["status"], "half_life_materialized_events": half["materialized_events"],
        "near_duplicate_pairs_abs_spearman_ge_0_90": len(near_dups), "strong_era_distribution_shift_features": strong_era,
        "era_interpretation": "Descriptive stability only; V1/V2 is confounded with calendar time/market maturity and is not a causal version-effect estimate.",
        "art029_rule": "Only PASS_COVERAGE features may be frozen; one interpretable regularized M_MOVE plus at most one nonlinear challenger; trial IDs before outcomes.",
        "outcomes_or_performance_read_by_script": False,
        "input_hashes": {"ic03_audit_ready_tape_sha256": sha256(tape_path), "ic04_yes_probability_trajectory_sha256": sha256(price_path), "pass_b_summary_sha256": sha256(pass_b_path)},
        "output_hashes": {"feature_matrix_sha256": sha256(matrix_gz), "feature_dictionary_sha256": sha256(root / "registry/art028_feature_dictionary.csv"),
                          "coverage_sha256": sha256(root / "registry/art028_feature_coverage.csv"), "era_stability_sha256": sha256(root / "registry/art028_era_stability.csv"),
    ...
    cov_lines = [f"- {r['feature_id']} {r['architecture_family']} ({r['role']}): {r['materialized_events']}/117 — `{r['status']}`" for r in coverage_rows]
...
- near-duplicate feature pairs |Spearman| >= 0.90: **{len(near_dups)}**
- strong descriptive V1/V2-era distribution shifts: **{len(strong_era)}** ({', '.join(strong_era) if strong_era else 'none'})
